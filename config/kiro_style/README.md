...
```
change_requests/{project_name}/          # one folder per project
  requirements.md
  design.md
  tasks.md
```
...
```
change_requests/{project_name}/sprint_{sprint_number}/
  requirements.md
  design.md
  tasks.md
```

Agents receive `project_name`, `sprint_number`, and `feature_request` in inputs; the path is `change_requests/{project_name}/requirements.md` (and same for design.md, tasks.md), or `change_requests/{project_name}/sprint_{sprint_number}/` when using sprint subfolders.
...
1. **Before implementing**: Read `change_requests/{project_name}/requirements.md` and `design.md` (and `tasks.md` if doing task execution). If these files do not exist for the current change request, the agent responsible for the phase MUST create them first (see tasks below).
...
- **SDLC crew**: Tasks `feature_triage_and_planning` and similar can be extended to also produce `requirements.md`, `design.md`, `tasks.md` in `change_requests/{project_name}/`, or new tasks can be added that run first and create these from `feature_request`.
- **Run inputs**: Ensure `project_name` and optionally `sprint_number` and `feature_request` are passed so agents know which change-request folder to use.