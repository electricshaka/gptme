...
- **Requirements:** `change_requests/{project_name}/requirements.md`
- **Design:** `change_requests/{project_name}/design.md`
- **Tasks:** `change_requests/{project_name}/tasks.md`

If sprint-specific folders are used: `change_requests/{project_name}/sprint_{sprint_number}/requirements.md` (and same for design.md, tasks.md).

Replace `{project_name}` and `{sprint_number}` with the actual values from your run inputs.
...
2. **If they do not exist**: Your planning phase MUST create them first using the templates in `config/kiro_style/templates/`. Use the `file_writer` tool to write:
   - `change_requests/{project_name}/requirements.md` – user stories, EARS acceptance criteria, MVP scope (max 5–7 MUST HAVE), future iterations.
   - `change_requests/{project_name}/design.md` – technical design, data flow, interfaces, architecture decisions, traceability to requirement IDs.
   - `change_requests/{project_name}/tasks.md` – ordered task list with dependencies, each task linked to requirement IDs and design sections, and an explicit per-task `Status`; include test and accessibility requirements.
